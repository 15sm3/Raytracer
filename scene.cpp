...

{
  ...
  if (g == 1 || glossyIterations == 1) {
    ...
    // YOUR CODE HERE
      ...
      vec3 u = R.perp1();
      vec3 v = R.perp2();
      vec3 colorsReflected = vec3(0.0, 0.0, 0.0);
      ...
      for (int i = 0; i<glossyIterations; i++) {
        ...
        vec3 Rp = (P + l*R + a*u + b*v);
        vec3 Pdir = (1.0 / sqrt(pow(Rp.x - P.x, 2) + pow(Rp.y - P.y, 2) + pow(Rp.z - P.z, 2) )) * (Rp - P);
        vec3 Iin = raytrace(P, Pdir, depth, objIndex, objPartIndex);
        colorsReflected = colorsReflected + Iin;
      }

    vec3 colorsReflected_Avg = (1.0f/glossyIterations) * colorsReflected;
    Iout = Iout + calcIout(N, R, E, E, kd, mat->ks, mat->n, colorsReflected_Avg);
    }
...
// YOUR CODE HERE
  float ni, nr;
  float cosI = (rayDir.normalize() * N.normalize());

  float dir = 1.0;
  float ratio, theta, ratioSin, theta2;

  if (rayDir.normalize() == N.normalize()) {
  ...
  theta = acos(cosI);
  ratioSin = (nr/ni)*(sin(theta));
  theta2 = asin(ratioSin);
  vec3 M = rayDir^N;

  if (nr == 1.008 && ratioSin > 1) {
    return false;
  }
  else {
    refractionDir = (dir * cos(theta2)* N.normalize()) + (ratioSin * (N^M).normalize());
  }
...
#else
 //std::cout << "new raytrace ________________________________________________________" << endl;
  ...
  for (int i = 0; i < numPixelSamples * numPixelSamples; i++) {  
      ...
      if(jitter){
         AA_x = static_cast <float>(rand()) / (RAND_MAX);
         AA_y = static_cast <float>(rand()) / (RAND_MAX);
      } else {
        AA_x = (i % numPixelSamples - static_cast <float>((numPixelSamples - 1)) / 2);
        AA_y = (i / numPixelSamples - static_cast <float>((numPixelSamples - 1)) / 2);
      }
      
      dir = (dir + ((AA_x / numPixelSamples) * right) + ((AA_y / numPixelSamples) * up)).normalize();
     
      total = total + raytrace(eye->position, dir, 0, -1, -1);
  }
  
  result = (1.0 / (numPixelSamples * numPixelSamples) ) * total; 
#endif
